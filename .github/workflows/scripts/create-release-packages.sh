...
build_variant() {
  ...
  if [[ -d templates ]]; then
    ...
    # macOS cp does not support --parents, so use tar for a portable copy.
    tar -C templates --exclude "commands" --exclude "vscode-settings.json" -cf - . \
      | tar -C "$SPEC_DIR/templates" -xf -
